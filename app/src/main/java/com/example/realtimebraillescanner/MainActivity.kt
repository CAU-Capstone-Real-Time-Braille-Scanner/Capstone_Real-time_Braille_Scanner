...
class MainActivity : AppCompatActivity() {
    ...
    private var REQUIRED_PERMISSIONS = arrayOf(Manifest.permission.CAMERA, Manifest.permission.INTERNET, Manifest.permission.RECORD_AUDIO)
    ...
    fun checkRunTimePermission() {
        //런타임 퍼미션 처리
        // 1. 카메라, 인터넷, 오디오 퍼미션을 가지고 있는지 체크합니다.
        val hasInternetPermission = ContextCompat.checkSelfPermission(this@MainActivity, Manifest.permission.INTERNET)
        val hasRecordPermission = ContextCompat.checkSelfPermission(this@MainActivity, Manifest.permission.RECORD_AUDIO)
        ...
        if (hasCameraPermission + hasRecordPermission + hasInternetPermission == PackageManager.PERMISSION_GRANTED) {
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(this@MainActivity, REQUIRED_PERMISSIONS.get(0))) {
                ...
                Toast.makeText(this, "카메라, 인터넷 접속, 오디오 녹음 등의 권한이 필요합니다.", Toast.LENGTH_LONG).show()
